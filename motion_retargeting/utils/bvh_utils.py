...
def format_offsets(hierarchy_str):
    formatted_lines = []
    for line in hierarchy_str.splitlines():
        if "OFFSET" in line:
            parts = line.strip().split()
            keyword = parts[0]
            floats = parts[1:]
            try:
                float_strs = ["%.6f" % float(val) for val in floats]
                indent = re.match(r"^\s*", line).group(0)
                new_line = f"{indent}{keyword} {' '.join(float_strs)}"
...
def save_bvh(filename, anim, names=None, frametime=1.0 / 24.0, order='zyx',
             ...
             end_offset=None, not_end_index=None, hierarchy_str=""):
    """
    ...
    with open(filename, 'w') as f:
        if hierarchy_str != "":
            HIERARCHY_STR = format_offsets(hierarchy_str)  
            HIERARCHY_STR = convert_hierarchy_to_cm(HIERARCHY_STR)
            f.write(HIERARCHY_STR + '\n')
            print("HIERARCHY_STR", HIERARCHY_STR)
        else:
            t = ""
            f.write("%sHIERARCHY\n" % t)
            f.write("%sROOT %s\n" % (t, names[0]))
