...
def dis_loss(pred, bool):
    device = pred.device
    loss = torch.nn.BCEWithLogitsLoss()
    if bool:
        gt = torch.ones_like(pred).to(device)
        return loss(pred, gt)
    else:
        gt = torch.zeros_like(pred).to(device)
        return loss(pred, gt)

def dis_loss_l2(pred, bool):
    device = pred.device
    if bool:
        gt = torch.ones_like(pred).to(device)
        return mse(pred, gt)
...
def caloutputloss(pred, gt, njoints, indices=None):
    pred_quat = pred[..., :njoints * 4].clone().reshape(pred.shape[0], pred.shape[1], njoints, 4)
    gt_quat = gt[..., :njoints * 4].clone().reshape(pred.shape[0], pred.shape[1], njoints, 4)
    if indices is None:
        quat_mse = torch.mean(mse(pred_quat, gt_quat))
    else:
        quat_mse = torch.mean(mse(pred_quat[..., indices, :], gt_quat[..., indices, :]))

    loss_total = quat_mse
    return loss_total


def calposloss(pred, gt, indices=None):
    if indices is None:
        return mse(pred, gt)
    else:
        return mse(pred[..., indices, :], gt[..., indices, :])


def cycle_latents(gan_model, src, tgt):
    return mse(gan_model.retar_latents[src], gan_model.cyc_latents[tgt])


def cycle_motions(gan_model, src, tgt, indice=None):
    if indice == None:
        return mse(gan_model.gt_pos[src], gan_model.cyc_pos[tgt])
    else:
        return mse(gan_model.gt_pos[src][..., indice, :], gan_model.cyc_pos[tgt][..., indice, :])
